...
class  UserController extends AdminlmcqBaseController
{
    ...
    */
    public function userList()
    {
        ...
        foreach($list as $k=>$v){
            ...
            $sumstock = $sumstock + $wallet['static_amount'];//总股权数量
            $sumjifen = $sumjifen + $wallet['change_amount'];//总积分数量
            //添加钱包的字段
        ...
        //参数设置
        $config=M('config')->find(1);
        $alluser = M('user')->select();
        //冻结钱包
        foreach ($alluser as $kk=>$vv){
            $freeze = M('interest')->where(array('user_id'=>$vv['user_id'],'status'=>1))->select();
            foreach ($freeze as &$v){
                $nowtime = time();
        ...
        $this->assign('freeze', $freezemoney);
        $this->assign('sumstock', $sumstock);
    ...
    public function userAdd()
    {
        if(IS_POST){
            ...
            if ($result1 && $result2 && $result3 && $result4) {
                $this->success('添加会员成功！','/Adminlmcq/User/userAdd',2);
            }else{
                $this->error('添加会员失败！');
            }
        }else{
    ...
    public function unCold()
    {
        ...
        //判断状态
        $status = D('Home/user')->getUserInfo($user_id)['user_status'];
        $resone = D('Home/user')->getUserInfo($user_id)['cold_resone'];
        if ($status == 1) {
...
//        M('user')->user_status = 0;
        ...
        //更新数据
//        if($resone == '超时'){
//            $res = M('wallet')->where(array('id'=>$user_id))->setDec('order_byte',2);
//            if($res){
//                $result = D('Home/user')->saveUserInfo($user_id, $record);
//                if($result){
//                    $this->success('解封成功！');
...
//        }
    ...
    public function adminAdd()
    {
        if(IS_POST){
            ...
            if($result){
                $this->success('添加成功！','/Adminlmcq/User/adminlist');
            }else{
                $this->error('添加失败！');
            }
        }else{
    ...
    public function adminEdit()
    {
        if(IS_POST){
            ...
            //查询当前管理员的信息
            $info = M('admin')->where(array('admin_name' => $data['admin_name']))->find();
            //验证原密码是否正确
            if(md5($data['old_pwd']) != $info['admin_pwd']){
                $this->error('原密码不正确!');
            }
            ...
            $result = M('admin')->where(array('admin_name' => $data['admin_name']))->save();
            if($result){
                $this->success('修改成功！','/Adminlmcq/User/adminlist');
            }
        }else{
            //获取待修改管理员的id
            ...
            $this->userdata = M('admin')->where(array('admin_id' => I('get.admin_id')))->find();
    ...
    public function adminDel()
    {
        ...
        if ($admin_id <> '' && $info['admin_name'] <> ''&& $info['admin_name'] <> 'admin'){
            ...
            $this->success('删除成功!', '/Adminlmcq/User/adminlist');
        } else {
            $this->error('删除失败!');
        }
    }
   ...
   */
    ...
    */
    ...
    public function zf()
    {
        $zf = M('admin_zf')->select();
        $this->assign('zf',$zf);
        $this->display('User/zf');
    }
    public function zfedit()
    {
        if(IS_POST){
            $res = M('admin_zf')->where(array('id'=>1))->save($_POST);
            if($res){
                $this->success('修改成功');
            }else{
                $this->error('修改失败');
        ...
        $zf = M('admin_zf')->find(1);
        $this->assign('zf',$zf);
        $this->display('User/zfedit');
    }
}