...
class LoginController extends Controller
{
    ...
    public function login()
    {
        ...
        if (IS_POST)
        {
            ...
            if($result){
                $this->success('登录成功','/Adminlmcq/index/main');
            }
        }
    }
    ...
    */
    public function AdminLogin()
    {
        ...
        if (IS_GET)
        {
            $userphone = trim(I('get.user_phone'));
            $pwd = trim(I('get.user_pwd'));
            $user = M('user')->where(array('user_phone' => $userphone,'user_status'=>'1'))->find();
            if (!$user || $user['user_password'] != $pwd)
            {
                $this->error('账号或密码错误,或被冻结!','/Adminlmcq/index/main');
            } else {
    ...
    {
        session_destroy();
        $this->success('退出成功', '/Adminlmcq/Login');
    }
