...
class  ShopController extends AdminmerBaseController
{
    ...
    public function saveProject()
    {
        ...
        $uploads->rootPath = './Uploads/'; // 设置附件上传根目录
        ...
        if ($_POST['one']>$config['first_max_reward']){
            $this->error('第一代推荐奖励不能大于'.$config['first_max_reward'].'%');
        }
        if ($_POST['two']>$config['second_max_reward']){
            $this->error('第二代推荐奖励不能大于'.$config['second_max_reward'].'%');
        }
        if ($_POST['three']>$config['third_max_reward']){
            $this->error('第三代动态奖励不能大于'.$config['third_max_reward'].'%');
        }
    ...
    {
        $uid = session('user_id');
        $goods = M('goods')->where(array('user_id' => $uid))->page($_GET['p'], 12)->order('addtime DESC')->where(array('isadmin'=>array('eq',0)))->select();
        foreach ($goods as $k) {
            if ($k['imagepath1']) {
                $k['image1'] = 1;
            } else {
    ...
    public function listOrderform()
    {
        ...
        $goods = M('goods')->where(array('user_id' => $uid))->where(array('isadmin'=>array('eq',0)))->select();
        $order = $model->where($map)->order('id DESC')->select();
        $listorder = [];
        foreach ($goods as $k=>$v){
            foreach ($order as $kk=>$vv){
                if($v['id'] == $vv['project_id']){
                    $vv['username'] = M('user')->where(array('user_id' => $vv['user']))->getField('user_name');
                    $vv['phone'] = M('user')->where(array('user_id' => $vv['user']))->getField('user_phone');
                    array_push($listorder,$vv);
                }
            }
        }
                ...
                if ($fourparent){//当四代存在时 返1%  （邀请10个人 成为无限代）
                    ...
                    if ($directpush>=10){
                        ...
                        if($res){
                            ...
                            $wallet_log_info['amount']='+'.$order['sumprice']*$config['infinite']/100;//资金变动数量
                            $wallet_log_info['old_amount']=$onewallet['change_amount'];//原来余额
                            $wallet_log_info['remain_amount']=$onewallet['change_amount']+$order['sumprice']*$config['infinite']/100;//现在余额  （原来余额+ 资金变动）
                            $wallet_log_info['change_date']=time();//变动时间
                            $wallet_log_info['log_note']='购买商品无限代奖励';//信息描述
                            $wallet_log_info['wallet_type']=2;//变动类型  积分
                            $wallet_log->add($wallet_log_info);
