 ...
 * Date:2017/03/31
 */
class SettingController extends AdminmerBaseController
{
    /*
     * 涨幅率设置
     */
        ...
        if(IS_POST){
           $rules=[
            ...
            if(!$config->validate($rules)->create()){
                ...
                if ($data['reward_rate1']>$config['first_max_reward']){
                    $this->error('第一代推荐奖励不能大于'.$config['first_max_reward'].'%');
                }
                if ($data['reward_rate2']>$config['second_max_reward']){
                    $this->error('第二代推荐奖励不能大于'.$config['second_max_reward'].'%');
                }
                if ($data['reward_rate3']>$config['third_max_reward']){
                    $this->error('第三代动态奖励不能大于'.$config['third_max_reward'].'%');
                }
                $editCon = M('mer_config')->where(array('user_id'=>$uid))->save($data);
                if($editCon){
                    $this->success('修改成功');
                }else{
                    $this->success('修改失败');
            ...
            $list = M('mer_config')->where(array('user_id'=>$uid))->find();
            if($list){
                $this->assign('list',$list);
                $this->display();
            }else{
                $add['user_id'] = $uid;
                $res = M('mer_config')->data($add)->add();
                $list = M('mer_config')->where(array('user_id'=>$uid))->find();
                $this->assign('list',$list);
                $this->display();
            }
